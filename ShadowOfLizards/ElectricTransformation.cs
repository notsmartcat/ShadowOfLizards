...
internal static class ElectricTransformation
{
    public static void Apply()
    {
        ...
        On.LizardSpit.ctor += LizardSpit_ctor;
        On.LizardSpit.Update += ElectricSpitSpit;
        On.LizardSpit.DrawSprites += LizardSpit_DrawSprites;
    ...
    #region Spit
    static void LizardSpit_ctor(On.LizardSpit.orig_ctor orig, LizardSpit self, Vector2 pos, Vector2 vel, Lizard lizard)
    {
        orig(self, pos, vel, lizard);

        if (!ShadowOfOptions.electric_transformation.Value || !ShadowOfOptions.electric_spit.Value || !lizardstorage.TryGetValue(self.lizard.abstractCreature, out LizardData data) || !data.liz.TryGetValue("ElectricColorTimer", out string timer) 
        ...
        ShockSpit.TryGetValue(self, out ElectricSpit data2);

        data2.ElectricColorTimer = graphicstorage.TryGetValue(lizard.graphicsModule as LizardGraphics, out GraphicsData data3) ? data3.ElectricColorTimer : 0;
    }

    static void LizardSpit_DrawSprites(On.LizardSpit.orig_DrawSprites orig, LizardSpit self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
        orig(self, sLeaser, rCam, timeStacker, camPos);

        if (!ShadowOfOptions.electric_transformation.Value || !ShadowOfOptions.electric_spit.Value || self == null || !ShockSpit.TryGetValue(self, out ElectricSpit data))
        {
            return;
        }
        ...
        if (data.ElectricColorTimer > 0)
        {
            ...
            Color color = Color.Lerp(data.origColor, Color.white, (float)data.ElectricColorTimer / 50f);
    ...
    static void ElectricSpitSpit(On.LizardSpit.orig_Update orig, LizardSpit self, bool eu)
    {
        ...
        if (!ShadowOfOptions.electric_transformation.Value || !ShadowOfOptions.electric_spit.Value || !ShockSpit.TryGetValue(self, out ElectricSpit data2) || self.lizard == null || !lizardstorage.TryGetValue(self.lizard.abstractCreature, out LizardData data)
            || data.transformation != "ElectricTransformation")
        {
            return;
        }

        if (self.stickChunk != null && self.stickChunk.owner.room == self.room && Custom.DistLess(self.stickChunk.pos, self.pos, self.stickChunk.rad + 40f) && self.fallOff > 0 && !data2.Shocked)
        {
            data2.Shocked = true;

            PhysicalObject owner = self.stickChunk.owner;
            self.room.PlaySound(SoundID.Centipede_Shock, self.pos);
            SpitSpark(data2, 1, true);
        ...
        if (!data2.Shocked)
        {
            for (int i = 0; i < 10; i++)
            {
                ...
                SpitSpark(data2, UnityEngine.Random.Range(0, 11), false);
            }
        }
        ...
        if (data2.lightSource != null)
        {
            ...
            data2.lightSource.setPos = new Vector2?(self.pos);
            data2.lightSource.setRad = new float?(300f * Mathf.Pow(data2.lightFlash * UnityEngine.Random.value, 0.1f) * Mathf.Lerp(0.5f, 2f, self.massLeft / 2));
        ...
        void SpitSpark(ElectricSpit data, int number, bool guarantee)
        {
            ...
            for (int i = 0; i < number; i++)
            {
                if (UnityEngine.Random.value < (data.ElectricColorTimer == 0 ? 0.025f : 0.025f * ((float)data.ElectricColorTimer / 50f)))
                {
                    Spark();
                }
            }
    ...
    static void LizardGraphics_Update(On.LizardGraphics.orig_Update orig, LizardGraphics self)
    {
        orig(self);

        if (!ShadowOfOptions.electric_transformation.Value || self == null || !lizardstorage.TryGetValue(self.lizard.abstractCreature, out LizardData data) || !data.liz.TryGetValue("ElectricColorTimer", out _)
    ...
    static void LizardGraphics_DrawSprites(On.LizardGraphics.orig_DrawSprites orig, LizardGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
        orig(self, sLeaser, rCam, timeStacker, camPos);
        ...
        if (data2.ElectricColorTimer > 0)
        {
            ...
            color = Color.Lerp(self.effectColor, Color.white, ((float)data2.ElectricColorTimer / 50f));
            ...
            //Debug.Log("Electric color timer = " + data2.ElectricColorTimer.ToString());
    ...
    static void SpearRecharge(On.MoreSlugcats.ElectricSpear.orig_Electrocute orig, MoreSlugcats.ElectricSpear self, PhysicalObject otherObject)
    {
        ...
        if (!ShadowOfOptions.electric_transformation.Value || otherObject == null || otherObject is not Lizard liz || !ShadowOfLizards.lizardstorage.TryGetValue(liz.abstractCreature, out LizardData data) || (data.transformation != "Electric" && data.transformation != "ElectricTransformation"))
        {
            return;
        }
    ...
    static void PlayerEatElectricLizard(On.Player.orig_EatMeatUpdate orig, Player self, int graspIndex)
    {
        ...
        if (!ShadowOfOptions.electric_transformation.Value || self.grasps[graspIndex] == null || self.grasps[graspIndex].grabbed is not Lizard liz || !graphicstorage.TryGetValue(liz.graphicsModule as LizardGraphics, out GraphicsData data2) || !lizardstorage.TryGetValue(liz.abstractCreature, out LizardData data) 
            || data.transformation != "ElectricTransformation" || self.eatMeat <= 40 || self.eatMeat % 15 != 3)
        {
            return;
        }
    ...
    static void ViolenceAttackerStun(On.Lizard.orig_Violence orig, Lizard self, BodyChunk source, Vector2? directionAndMomentum, BodyChunk hitChunk, PhysicalObject.Appendage.Pos onAppendagePos, Creature.DamageType type, float damage, float stunBonus)
    {
        ...
        if (!ShadowOfOptions.electric_transformation.Value || !lizardstorage.TryGetValue(self.abstractCreature, out LizardData data) || !graphicstorage.TryGetValue(self.graphicsModule as LizardGraphics, out GraphicsData data2) || source == null || source.owner == null || data.transformation != "ElectricTransformation")
        {
            return;
        }
        ...
        LizardSpark(self, hitChunk, data2, 1, true);
    ...
    static void LizardUpdate(On.Lizard.orig_Update orig, Lizard self, bool eu)
    {
        ...
        if (!ShadowOfOptions.electric_transformation.Value || self == null || self.abstractCreature == null || !graphicstorage.TryGetValue(self.graphicsModule as LizardGraphics, out GraphicsData data2) || !lizardstorage.TryGetValue(self.abstractCreature, out LizardData data) 
            || data.transformation != "ElectricTransformation")
        {
            return;
        }
    ...
    {
        if (!ShadowOfOptions.electric_transformation.Value || self == null || !graphicstorage.TryGetValue(self.graphicsModule as LizardGraphics, out GraphicsData data2) || !lizardstorage.TryGetValue(self.abstractCreature, out LizardData data) || data.transformation != "ElectricTransformation")
        {
    ...
    static void LizardSpark(Lizard self, BodyChunk chunk, GraphicsData data, int number, bool guarantee)
    {
        ...
        for (int i = 0; i < number; i++)
        {
            if (UnityEngine.Random.value < (data.ElectricColorTimer == 0 ? 0.025f : 0.025f * ((float)data.ElectricColorTimer / 50f)))
            {
                Debug.Log(data.ElectricColorTimer == 0 ? 0.025f : 0.025f * ((float)data.ElectricColorTimer / 50f));
                Spark();
            }
        }
